...
<%@ include file="../includes/header.jsp" %>
        ...
        <div id="page-wrapper">
			...
			<div>
				카메라모아에 등록 된 공지사항 목록입니다. 글을 작성하거나, 제목을 클릭하여 내용을 수정하거나 삭제할 수 있습니다.
			</div>
			<br><br>
			...
			<table>
				...
				<tbody>
					...
					<tr>
						<td>${notice.nbno}</td>
						<td>
							<a href="/admin/board/noticeGet?n=${notice.nbno}">${notice.ntitle }</a>
						</td>
						<td>${notice.nwriter}</td>
						<td>
							<fmt:formatDate value="${notice.nregdate}" pattern="yyyy-MM-dd"/>
						</td>
						<td>
							<fmt:formatDate value="${notice.nupdateDate}" pattern="yyyy-MM-dd" />
						</td>
					</tr>
					<c:if test="${status.last }">
					<tr>
						<td colspan="5" style="text-align:right;">
						총 공지사항 갯수: <span style="color:red;">${status.count}</span>개
						</td>
					</tr>
					</c:if>
					</c:forEach>
					<tr>
						<td colspan="5" style="text-align:right;">
						<button type="button" id="register_Btn" class="btn btn-info">글쓰기</button>
						
						<script>
							$("#register_Btn").click(function(){
								location.href = "/admin/board/noticeRegister";
							});
						</script>
