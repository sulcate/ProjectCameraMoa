...
<%@ include file="../includes/header.jsp" %>
        ...
        <div id="page-wrapper">
			...
			<div>
				카메라모아에서 환불 대기중인 목록 및 처리중인 목록 입니다. 주문번호를 클릭해 상세페이지에서 환불 요청 상품의 수량을 보거나 상태를 변경할 수 있습니다.
			</div>
			<br><br>
           		<div id="container_box">
           		<table>
           			<caption>환불 대기중인 목록</caption>
				...
				<tbody>
					<c:set var="i" value="0" />
					<c:forEach items="${refundList }" var="refundList" varStatus="status">
					<c:if test="${refundList.state == '환불 신청'}">
					<c:set var="i" value="${i + 1}"/>
					<tr>
							<td>
							<a href="/admin/shop/refundView?n=${refundList.orderId}">${refundList.orderId}</a>
							</td>			
						<td><fmt:formatDate value="${refundList.refundDate}" pattern="yyyy.MM.dd" /></td>
						<td>${refundList.userId}</td>
						<td>${refundList.orderRec}</td>
						<td>
							(${refundList.postCode}) ${refundList.userAddr1} ${refundList.userAddr2} ${refundList.userAddr3}
						</td>
						<td>
							<fmt:formatNumber pattern="###,###,###" value="${refundList.amount}" /> 원
						</td>
						<td>${refundList.state }</td>
					</tr>
					</c:if>
					<c:if test="${status.last }">
					<tr>
						<td colspan="7" style="text-align:right;">
						총 대기 건수: <span style="color:red;"><c:out value="${i}" /></span>개
						</td>
					</tr>
					</c:if>
					</c:forEach>
			...
			<table>
				...
				<tbody>
					<c:set var="j" value="0" />
					<c:forEach items="${refundList }" var="refundList" varStatus="status">
					<c:if test="${refundList.state == '환불 진행중'}">
			...
			<table>
				...
				<tbody>
					<c:set var="k" value="0" />
					<c:forEach items="${refundList }" var="refundList" varStatus="status">
					<c:if test="${refundList.state == '환불 완료' or refundList.state == '환불 불가' }">
					...
					<tr>
						<td colspan="7" style="text-align:right;">
						총 환불 건수: <span style="color:red;"><c:out value="${k}" /></span>개
						</td>
					</tr>
					</c:if>
					</c:forEach>
